<script lang="ts">
	...
	type NavItem = { text: string; link?: string; target?: string; items?: NavItem[] };
	...
	function flatItems(items: NavItem[], depth = 0): FlatItem[] {
		...
		for (const it of items) {
			if (it.items?.length) {
				if (depth === 0 && out.length > 0) out.push({ kind: 'divider' });
	...
	let { rootPath, mainTitle = 'Greg', version = '', nav = [], theme, onThemeChange, navigate, onOpenSearch }: Props = $props();
</script>

<svelte:window onclick={() => { openDropdownIdx = null; }} />
		...
		{#each nav as item, i}
			{#if item.items?.length}
				...
				>
					<button
						...
						type="button"
						onclick={() => { openDropdownIdx = openDropdownIdx === i ? null : i; }}
						aria-expanded={openDropdownIdx === i}
						aria-haspopup="menu"
					>
						{item.text}
						<svg class="dropdown-chevron" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg>
					</button>
							...
							{:else if ri.kind === 'header'}
								{#if ri.item.link}
									<a
										...
										class:nav-dropdown-group--nested={ri.depth > 0}
										role="menuitem"
										target={ri.item.target ?? (EXTERNAL_RE.test(ri.item.link) ? '_blank' : undefined)}
										rel={EXTERNAL_RE.test(ri.item.link) ? 'noopener noreferrer' : undefined}
										onclick={(e) => { openDropdownIdx = null; if (!EXTERNAL_RE.test(ri.item.link!) && !ri.item.target) { e.preventDefault(); navigate(ri.item.link!); } }}
									>{ri.item.text}</a>
								{:else}
									<p class="nav-dropdown-group" class:nav-dropdown-group--nested={ri.depth > 0}>{ri.item.text}</p>
								{/if}
							{:else}
								<a
									href={ri.item.link ?? '#'}
									class="nav-dropdown-item"
									class:nav-dropdown-item--nested={ri.depth > 0}
									role="menuitem"
									target={ri.item.target ?? (ri.item.link && EXTERNAL_RE.test(ri.item.link) ? '_blank' : undefined)}
									rel={ri.item.link && EXTERNAL_RE.test(ri.item.link) ? 'noopener noreferrer' : undefined}
									onclick={(e) => { openDropdownIdx = null; if (ri.item.link && !EXTERNAL_RE.test(ri.item.link) && !ri.item.target) { e.preventDefault(); navigate(ri.item.link); } }}
								>{ri.item.text}</a>
							{/if}
						{/each}
						</div>
			...
			{:else}
				<a
					href={item.link ?? '#'}
					class="nav-link"
					target={item.target ?? (item.link && EXTERNAL_RE.test(item.link) ? '_blank' : undefined)}
					rel={item.link && EXTERNAL_RE.test(item.link) ? 'noopener noreferrer' : undefined}
					onclick={(e) => { if (item.link && !EXTERNAL_RE.test(item.link) && !item.target) { e.preventDefault(); navigate(item.link); } }}
				>{item.text}</a>
			{/if}
		{/each}
	</nav>
