<script lang="ts">
	...
	function getOutlineHeadingText(heading: HTMLElement): string {
		...
		for (const marker of Array.from(
			clone.querySelectorAll("a.header-anchor"),
		)) {
			marker.remove();
		}
	...
	function doScan() {
		if (!tagSelector || !container) {
			items = [];
			return;
		}
		const nodes = Array.from(
			container.querySelectorAll<HTMLElement>(tagSelector),
		);
		items = nodes
			.filter((n) => n.id)
			.map((n) => ({
	...
	$effect(() => {
		...
		container.addEventListener("scroll", scheduleActiveUpdate, {
			passive: true,
		});
	...
	const minDepth = $derived(
		items.length ? Math.min(...items.map((i) => i.depth)) : 2,
	);
</script>
